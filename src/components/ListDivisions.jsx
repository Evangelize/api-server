...
@inject('classes')
@observer
class ListDivisions extends Component {
  ...
  render() {
    const { classes, yearId, onEdit, sortable } = this.props;
    let retVal = (
      <List>
      {classes.getDivisionSchedulesByPosition(yearId).map((item) =>
        <Division key={item.id} item={item} onEdit={onEdit} />
      )}
      </List>
    );
    if (this.props.sortable) {
       retVal = (<SortableComponent items={classes.getDivisionSchedulesByPosition(yearId)} onSortEnd={this.moveItem} />);
    }
    return retVal;
  }
}
